...
class VerifyCode:
    def dispatch(self, request, *args, **kwargs):
        print(request.COOKIES.get('CsrfToken')[:10])
        print(request.COOKIES.get('CsrfToken'))
        print(self.data['captcha']['code'])
        print('hget',type(Rdb.hget(request.COOKIES.get('CsrfToken')[:10],'verifyCode')))
        print('post',type(self.data['captcha']['code']))
        print(self.data['captcha']['code'] == Rdb.hget(request.COOKIES.get('CsrfToken')[:10],'verifyCode'))
        if Rdb.hget(request.COOKIES.get('CsrfToken')[:10],'verifyCode') != self.data['captcha']['code']:
...
def getCaptcha(request):
    ...
    Rdb.hset(request.COOKIES.get('CsrfToken')[:10], 'verifyCode',data)
    print(request.COOKIES.get('CsrfToken')[:10])
    print(request.COOKIES.get('CsrfToken'))
    print(data)
    return JsonResponse({
